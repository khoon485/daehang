...
class GameMacro:
    ...
    def do_haggle(self):
        ...
        # ==========================================
    def do_eat(self):
        """밥 먹기 로직: 퀵슬롯 열고 4번 키 11회 입력"""
        print(f"\n[🍴] 밥 먹기 타임! (사이클 {self.cycle_count})")

        # 1. 퀵슬롯 열기 - ` 키 (백틱, 틸드 키)
        print(f"    [1] 퀵슬롯 열기 (` 키)")
        self.press_key(0xC0, delay_after=2.0)  # 퀵슬롯 열리는 시간 충분히 대기
        if not self.check_hotkeys(): return False

        # 2. '4' 키 11번 누르기
        for i in range(11):
            print(f"    - 밥 먹는 중... ({i+1}/11)")
            self.press_key(0x34, delay_after=0.5)  # 밥 먹는 애니메이션 충분히 대기
            if not self.check_hotkeys(): return False
